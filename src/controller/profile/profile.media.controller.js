...
exports.getProfileMedia = async (req, res) => {
  try {
    ...
    let sql = `select users.id, users.name, users.username, users.profile_img_url, tweets.content as tweetContent, tweets.id as tweet_id, medias.media_url, tweet_likes.status as isLiked, bookmarks.status as isBookmarked, tweets.created_at as tweet_time,  retweets.deleted_at as notRetweeted, retweets.created_at as createdAt, retweets.retweet_message as retweetMsg from tweets join users on users.id = tweets.user_id inner join medias on medias.tweet_id = tweets.id left join tweet_likes on tweet_likes.tweet_id = tweets.id and tweet_likes.user_id = ?
