  ...
  try {
    ...
    if (userProfileData) {
      ...
      if (postData[0]) {
        ...
        res.render("../views/pages/profile", {id: req.query.id,user: req.user[0][0],userId,id,userProfileData,followingDetailData,followerDetailData,profileData,twitCountData,replyData,postData,isFollowing,tweetData,commentData,followerId,});
      } else {
        const followingDetail = `select count(*) as following from followers where following_id=? and is_blocked =0 and current_status='1' `;
        const [followingDetailData] = await connection.query(followingDetail,[id]);
        ...
        res.render("../views/pages/profile", {id: req.query.id,user: req.user[0][0],userId,id,userProfileData,followingDetailData,followerDetailData,profileData,twitCountData,replyData,isFollowing,postData,followerId,tweetData,commentData});
      ...
      res.render("../views/pages/profile", {id: req.query.id, user: req.user[0][0],userId,id,userProfileData,followingDetailData,followerDetailData,profileData,twitCountData,isFollowing,replyData,tweetData,followerId: null,commentData});
    }
  } catch (error) {
    console.log(error);
  }
